export const general = {
  title: 'Minifestival',
  date: '10.10.2017',
  location: 'Aula des Marie-Curie-Gymnasiums',
  client: 'Abijahrgang 2017',
  contact: 'Marc Nitzsche, Flo Geißler',
};

export const schedule = {
  start: '22.02.2017 18:00 Uhr',
  rehearsal: '22.02.2017 12:30 Uhr',
  construction: '22.02.2017 07:30 Uhr',
  deconstruction: '22.02.2017 20:30 Uhr, 23.02.2017 08:00 Uhr',
};

export const requirements = {
  Ton: 'Dynamische und Kondensatormikrofone, E- Piano, Verstärker, Monitore, Schlagzeugmikrofone',
  Licht: 'LEDs, Stufenlinsen, Gassenlicht, Jands Vista',
  Bühne: 'Normaler Aufbau',
};

export const additionalDetails = {
  Veranstaltungskonzept: 'Die geilsten Künstler der Schule tun sich zusammen um ein Festival zu organisieren, das einem Wohnzimmerkonzert in nichts nachsteht!',
  Besonderheiten: 'Keine Moderation, Acts kündigen sich selber an. Wohnzimmerathmosphäre.',
};

export const actions = [
  {
    id: 'RANDOMKEY01',
    fields: {
      title: 'Gravity',
      description: 'Song von Sara Bareilles',
      length: '5 Minutes',
      artist: 'Rebecca Schönknecht',
      requirements: '1 HH, Piano',
    },
  },
  {
    id: 'RANDOMKEY02',
    fields: {
      title: 'Imagine',
      description: 'Song von John Lennon',
      length: '3 Minutes',
      artist: 'Flo Geißler, Vera Gennrich',
      requirements: '2 HH, DI-Box',
    },
  },
  {
    id: 'RANDOMKEY03',
    fields: {
      title: 'Probleme',
      description: 'Das ist echte Lyrik.',
      length: '4 Minutes',
      artist: 'Lucas Schemenz',
      requirements: '1 HH, Mikrofonständer',
    },
  }
];
